...
<div class="hdr"><h1><span>&#9670;</span> SCOUT</h1><button class="btn btn-p" onclick="openForm()">+ Add</button></div>
...
async function load(){var r=await fetch(A+'/targets').then(function(r){return r.json()});items=r.targets||[];document.getElementById('stats').innerHTML='<div class="st"><div class="st-v">'+items.length+'</div><div class="st-l">Total</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div>';render();}
function render(){var q=(document.getElementById('search').value||'').toLowerCase();var f=items;
if(q)f=f.filter(function(i){return (i.name||'').toLowerCase().includes(q)||(i.url||'').toLowerCase().includes(q)||(i.type||'').toLowerCase().includes(q)||(i.interval_seconds||'').toLowerCase().includes(q)});
...
if(i.interval_seconds)h+='<span>'+esc(i.interval_seconds)+'</span>';
if(i.status)h+='<span class="badge">'+esc(i.status)+'</span>';
if(i.last_check)h+='<span>'+esc(i.last_check)+'</span>';
if(i.last_result)h+='<span>'+esc(i.last_result)+'</span>';
if(i.fail_count)h+='<span>'+esc(i.fail_count)+'</span>';
h+='<span>'+ft(i.created_at)+'</span></div>';

h+='</div>';});document.getElementById('list').innerHTML=h;}
async function del(id){if(!confirm('Delete?'))return;await fetch(A+'/targets/'+id,{method:'DELETE'});load();}
function formHTML(item){var i=item||{name:"",url:"",type:"",interval_seconds:"",status:"",last_check:"",last_result:"",fail_count:""};var isEdit=!!item;
var h='<h2>'+(isEdit?'EDIT':'NEW')+' SCOUT</h2>';
...
h+='<div class="fr"><label>Interval</label><input id="f-interval-seconds" value="'+esc(i.interval_seconds)+'"></div>';
h+='<div class="fr"><label>Status</label><input id="f-status" value="'+esc(i.status)+'"></div>';
h+='<div class="fr"><label>Last Check</label><input id="f-last-check" value="'+esc(i.last_check)+'"></div>';
h+='<div class="fr"><label>Last Result</label><input id="f-last-result" value="'+esc(i.last_result)+'"></div>';
h+='<div class="fr"><label>Fail Count</label><input id="f-fail-count" value="'+esc(i.fail_count)+'"></div>';
...
async function submit(){var body={name:document.getElementById('f-name').value.trim(),url:document.getElementById('f-url').value.trim(),type:document.getElementById('f-type').value.trim(),interval_seconds:document.getElementById('f-interval-seconds').value.trim(),status:document.getElementById('f-status').value.trim(),last_check:document.getElementById('f-last-check').value.trim(),last_result:document.getElementById('f-last-result').value.trim(),fail_count:document.getElementById('f-fail-count').value.trim()};
