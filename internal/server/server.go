package server
import ("encoding/json";"net/http";"github.com/stockyard-dev/stockyard-scout/internal/store")
...
func(s *Server)routes(){
    ...
    s.mux.HandleFunc("POST /api/sites/{id}/crawl",s.handleCrawl)
    s.mux.HandleFunc("GET /api/sites/{id}/runs",s.handleListRuns)
    s.mux.HandleFunc("GET /api/runs/{id}/issues",s.handleListIssues)
    s.mux.HandleFunc("GET /",s.handleUI)
}
func(s *Server)handleHealth(w http.ResponseWriter,r *http.Request){writeJSON(w,200,map[string]string{"status":"ok","service":"stockyard-scout"})}
func(s *Server)handleVersion(w http.ResponseWriter,r *http.Request){writeJSON(w,200,map[string]string{"version":"0.1.0","service":"stockyard-scout"})}
