...
func main(){port:=os.Getenv("PORT");if port==""{port="9700"};dataDir:=os.Getenv("DATA_DIR");if dataDir==""{dataDir="./scout-data"}
db,err:=store.Open(dataDir);if err!=nil{log.Fatalf("scout: %v",err)};defer db.Close();srv:=server.New(db,server.DefaultLimits())
